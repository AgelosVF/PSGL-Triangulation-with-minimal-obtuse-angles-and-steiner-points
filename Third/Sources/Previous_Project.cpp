...
//-----------------------------------------------------------------------------------------
void old_reduce_obtuse_midpoint_steiner(Custom_CDT &cdel_tri, const Polygon_2 &region_boundary, boost::associative_property_map<std::unordered_map<Face_handle, bool>> &in_domain){
			...
			if(get(in_domain,f) && is_obtuse_triangle(f) ){
				...
				if( local_f != Face_handle() ){
					...
					if(new_count<og_obtuse_count){
						std::cout<<"Added steiner on midpoint\n";
...
void old_reduce_obtuse_circumcenter_steiner(Custom_CDT &cdel_tri, const Polygon_2 &region_boundary, boost::associative_property_map<std::unordered_map<Face_handle, bool>> &in_domain){
	bool steiner_added=true;
...
void old_reduce_obtuse_projection_steiner(Custom_CDT &cdel_tri, const Polygon_2 &region_boundary, boost::associative_property_map<std::unordered_map<Face_handle, bool>> &in_domain){
	bool steiner_added=true;
...
void previous_triangulation(Custom_CDT& cdel_tri,const Polygon_2& region_polygon){
	...
	while(reduced){
		...
		old_reduce_obtuse_projection_steiner(cdel_tri, region_polygon, in_domain);
		CGAL::mark_domain_in_triangulation(cdel_tri, in_domain);
		new_obtuse=count_obtuse_faces(cdel_tri,in_domain);
		if(new_obtuse<num_obtuse)
			reduced=true;
		//try circumcenter steiner
		old_reduce_obtuse_circumcenter_steiner(cdel_tri,region_polygon,in_domain);
		CGAL::mark_domain_in_triangulation(cdel_tri, in_domain);
		new_obtuse=count_obtuse_faces(cdel_tri,in_domain);
		if(new_obtuse<num_obtuse)
			reduced=true;
		//try midpoint of largest edge steiners
		old_reduce_obtuse_midpoint_steiner(cdel_tri,region_polygon, in_domain);
		new_obtuse=count_obtuse_faces(cdel_tri,in_domain);
		if(new_obtuse<num_obtuse)
			reduced=true;
		CGAL::mark_domain_in_triangulation(cdel_tri, in_domain);
